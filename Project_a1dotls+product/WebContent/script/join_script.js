$(function(){
	...
	$("button#dupChkBtn").click(function(){
		let uId = $("div#wrap_Popup input#uId").val().trim();
		$("input#uId").val(uId);
		...
		if (uId == "") {    // 전체 공백 체크
			...
			$("form#idChkFrm").attr("action", "/join/idCheckProc.jsp?uId=" + uId).submit();
	...
	$("button#chkIdPopupBtn").click(function(){
		// 기능 확인하기
		let btnCont = $("button#chkIdPopupBtn").text();
		if(btnCont == "재입력"){
			window.location.href = "/join/idCheck.jsp";
		} else if (btnCont == "사용"){
			window.close();
			// opener.joinFrm.uId.value = $("input#uId").val();
			$("#uId", opener.document).val($("#uId").val());
		}
	...
	$("#uPw").keyup(function(){
		...
		if(uPw == null){
			$("span#uPwPattern").text("영어 대소문자/숫자/특수문자 조합, 8~16자");
	...
	$("#uPwRe").keyup(function(){
		...
		if(uPw != uPwRe){
			$("span#uPwChk").text("비밀번호가 일치하지 않습니다.").css("color", "red");
		} else{
			$("span#uPwChk").text("비밀번호가 일치합니다.").css("color", "black");;
		}
	...
	function fnJoinSbm(){
		...
		let chkReq0 = $(".joinAgree .termArea").find("input").prop("checked"); // 필수 체크 확인
		let chkReq1 = $(".joinAgree .termArea").find("input").prop("checked"); // 필수 체크 확인
